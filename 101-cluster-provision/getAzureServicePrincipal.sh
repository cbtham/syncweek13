...
if [ ! -z "$DEFAULT_ACCOUNT_ID" ]; then
    ...
    if [ -z "$selection" ]; then
        ...
        echo "Overriding variables to sp.env run 'source sp.env'"
        echo "export AZURE_SUBSCRIPTION_ID=${AZURE_SUBSCRIPTION_ID}" > sp.env
        echo "export AZURE_CLIENT_ID=${AZURE_CLIENT_ID}" >> sp.env
        echo "export AZURE_CLIENT_NAME=${AZURE_CLIENT_NAME}" >> sp.env
        echo "export AZURE_CLIENT_SECRET=${AZURE_CLIENT_SECRET}" >> sp.env
        echo "export AZURE_TENANT_ID=${AZURE_TENANT_ID}" >> sp.env
